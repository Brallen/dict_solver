...
function split2(splitter, string5) {
  const match = string5.match(splitter);
  if (!match) return [string5, "", ""];
...
var MissingClosingQuote = class extends CustomType {
  constructor(starting_line) {
    super();
    this.starting_line = starting_line;
  }
};
var Splitters = class extends CustomType {
  constructor(separator, newlines, separator_or_quote, quotes) {
    super();
    this.separator = separator;
    this.newlines = newlines;
    this.separator_or_quote = separator_or_quote;
    this.quotes = quotes;
  }
};
function escaped_loop(loop$line, loop$newline, loop$rest, loop$splitters, loop$start, loop$line_number, loop$field) {
  while (true) {
    let line = loop$line;
    let newline = loop$newline;
    let rest = loop$rest;
    let splitters = loop$splitters;
    let start4 = loop$start;
    let line_number = loop$line_number;
    let field2 = loop$field;
    let $ = split2(splitters.quotes, line);
    let $1 = $[1];
    if ($1 === "") {
      let $2 = $[2];
      if ($2 === "") {
        let line_piece = $[0];
        let $3 = split2(splitters.newlines, rest);
        let $4 = $3[0];
        if ($4 === "") {
          let $5 = $3[1];
          if ($5 === "") {
            let $6 = $3[2];
            if ($6 === "") {
              return new Error(new MissingClosingQuote(start4));
            } else {
              let line$1 = $4;
              let rest_newline = $5;
              let rest$1 = $6;
              let line$2 = line_piece + newline + line$1;
              let line_number$1 = line_number + 1;
              loop$line = line$2;
              loop$newline = rest_newline;
              loop$rest = rest$1;
        ...
        let field_piece = $[0];
        let line$1 = $2;
        let $3 = starts_with(line$1, splitters.separator);
        if ($3) {
          let line$2 = split2(splitters.separator_or_quote, line$1)[2];
          let _block;
          if (field2 === "") {
            _block = field_piece;
          } else {
            _block = field2 + field_piece;
          }
          let field$1 = _block;
          return new Ok([line$2, newline, rest, line_number, field$1]);
        } else {
          return new Error(new UnescapedQuote(line_number));
        }
      }
    } else if ($1 === '""') {
      let field_piece = $[0];
      let line$1 = $[2];
      let field$1 = field2 + field_piece + '"';
      loop$line = line$1;
      loop$newline = newline;
      loop$rest = rest;
      loop$splitters = splitters;
      loop$start = start4;
      loop$line_number = line_number;
      loop$field = field$1;
    } else {
      let $2 = $[2];
      if ($2 === "") {
        ...
        let field$1 = _block;
        return new Ok(["", newline, rest, line_number, field$1]);
      } else {
        let field_piece = $[0];
        let line$1 = $2;
        let $3 = starts_with(line$1, splitters.separator);
...
function field_loop(loop$line, loop$newline, loop$rest, loop$splitters, loop$line_number, loop$fields) {
  while (true) {
    let line = loop$line;
    let newline = loop$newline;
    let rest = loop$rest;
    ...
    let $ = split2(splitters.separator_or_quote, line);
    let $1 = $[1];
    if ($1 === "") {
      let $2 = $[2];
      if ($2 === "") {
        let field2 = $[0];
        return new Ok(
          [rest, line_number, reverse(prepend(field2, fields))]
        );
      } else {
        let field2 = $[0];
        let line$1 = $2;
        let fields$1 = prepend(field2, fields);
        loop$line = line$1;
        loop$newline = newline;
        loop$rest = rest;
        loop$splitters = splitters;
        loop$line_number = line_number;
        loop$fields = fields$1;
      }
    } else if ($1 === '"') {
      let $2 = $[0];
      if ($2 === "") {
        let line$1 = $[2];
        let start4 = line_number;
        let field2 = escaped_loop(
          line$1,
          newline,
          rest,
          splitters,
        ...
        if (field2 instanceof Ok) {
          let $3 = field2[0][0];
          if ($3 === "") {
            let rest$1 = field2[0][2];
            let line_number$1 = field2[0][3];
            let field$1 = field2[0][4];
            return new Ok(
              [
                rest$1,
                line_number$1,
                reverse(prepend(field$1, fields))
              ]
            );
          } else {
            let line$2 = $3;
            let newline$1 = field2[0][1];
            let rest$1 = field2[0][2];
            let line_number$1 = field2[0][3];
            let field$1 = field2[0][4];
            let fields$1 = prepend(field$1, fields);
            loop$line = line$2;
            loop$newline = newline$1;
            loop$rest = rest$1;
            loop$splitters = splitters;
            loop$line_number = line_number$1;
            loop$fields = fields$1;
...
function line_loop(loop$csv, loop$splitters, loop$line_number) {
  while (true) {
    let csv = loop$csv;
    let splitters = loop$splitters;
    let line_number = loop$line_number;
    let $ = split2(splitters.newlines, csv);
    let $1 = $[0];
    if ($1 === "") {
      let $2 = $[2];
      if ($2 === "") {
        return new Ok(["", line_number, toList([])]);
      } else {
        let rest = $2;
        loop$csv = rest;
        loop$splitters = splitters;
...
function lines_loop(loop$csv, loop$splitters, loop$line_number, loop$lines) {
  while (true) {
    let csv = loop$csv;
    let splitters = loop$splitters;
    let line_number = loop$line_number;
    ...
    if (csv === "") {
      return new Ok(reverse(lines));
    } else {
      let $ = line_loop(csv, splitters, line_number);
      if ($ instanceof Ok) {
        let rest = $[0][0];
        let line_number$1 = $[0][1];
        let line = $[0][2];
        loop$csv = rest;
        loop$splitters = splitters;
        loop$line_number = line_number$1;
        loop$lines = prepend(line, lines);
      } else {
        return $;
      }
    }
...
function to_lists(csv, separator) {
  let splitters = new Splitters(
    separator,
    new$(toList(["\r\n", "\n"])),
    new$(toList([separator, '"'])),
    new$(toList(['""', '"']))
  );
  return lines_loop(csv, splitters, 0, toList([]));
}
...
var Model = class extends CustomType {
  constructor(bank, options, word_letters, invalid_letters) {
    super();
    this.bank = bank;
    this.options = options;
    this.word_letters = word_letters;
    this.invalid_letters = invalid_letters;
  }
};
...
var InvalidLettersInput = class extends CustomType {
  constructor(value) {
    super();
    this.value = value;
  }
...
function fetch_csv() {
  ...
  let handler = expect_text(
    (response) => {
      if (response instanceof Ok) {
        let results = response[0];
        let _block;
        let _pipe = to_lists(results, ",");
        let _pipe$1 = map3(_pipe, flatten);
        _block = unwrap2(_pipe$1, toList([]));
        let res = _block;
        return new ApiReturnedCSV(new Ok(res));
      } else {
        return new ApiReturnedCSV(new Ok(toList([])));
      }
    }
  );
...
function update2(model, msg) {
  if (msg instanceof ApiReturnedCSV) {
    ...
    return [
      new Model(
        model.bank,
        get_options(value, model.bank, model.invalid_letters),
        value,
        model.invalid_letters
      ),
      none()
    ...
    toList([
      div(
        ...
        toList([
          ...
          div(
            ...
            toList([
              label(
                toList([for$("invalid_letters_input")]),
                toList([text2("Invalid Letters")])
              ),
              input(
                toList([
                  name("invalid_letters_input"),
                  class$("invalid_letters_input"),
                  on_input(
                    (value) => {
                      return new InvalidLettersInput(value);
                    }
                  )
                ])
              )
      ...
      div(
        ...
        map(
          model.options,
          (option) => {
            return p(toList([]), toList([text2(option)]));
          }
        )
      )
    ])
